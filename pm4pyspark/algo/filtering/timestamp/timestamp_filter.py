...
def filter_traces_intersecting(df, dt1, dt2, parameters=None):
    ...
    stacked3 = stacked3.filter(stacked2[timestamp_key + "_last"] > dt2)
    stacked.unpersist()

    stacked = stacked1.union(stacked2)
    stacked = stacked.union(stacked3)
    stacked = stacked.drop(timestamp_key + "_last", timestamp_key + "_first").distinct().orderBy(case_id_glue, timestamp_key)
