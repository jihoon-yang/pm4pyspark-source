...
def apply(df, paths, parameters=None):
    ...
    df_reduced = df.select(case_id_glue, attribute_key)
    ...
    stacked_df = stacked_df.filter(stacked_df["@@path"].isin(paths)).select(case_id_glue)

    if positive:
        return df.join(F.broadcast(stacked_df), case_id_glue)
    else:
        return df.join(F.broadcast(stacked_df), case_id_glue, 'leftanti')
