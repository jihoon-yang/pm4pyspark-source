...
#df = importer.import_sparkdf_from_path_wo_timeconversion(os.path.join(INPUT_DATA_DIR, "running-example.csv"), header=True, inferSchema=True)
...
#filtered_df = attributes_filter.filter_df_on_attribute_values(spark_df, {'examine casually'})

#filtered_num_df = attributes_filter.apply_numeric_events(spark_df, 50, 100, parameters={"pm4py:param:attribute_key":"Costs"})

#filtered_num_tr_df = attributes_filter.apply_numeric(spark_df, 50, 100, parameters={"pm4py:param:attribute_key":"Costs"})
...
print(attributes_filter.get_kde_date_attribute(spark_df)) #ERROR TIME DIFFERENCE...



#filtered_top_5_act_df.show(filtered_top_5_act_df.count())
spark_df.unpersist()
